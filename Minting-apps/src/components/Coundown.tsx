...
const CountdownTimer = () => {
  ...
  const { data: mintTimes } = useReadContract({
    address: ADDRESS,
    abi: ABI,
  ...
  if (!mintTimes) return null;

  const [freemintStart, whitelistStart, publicStart] = mintTimes as [
    bigint,
    bigint,
    bigint
  ];
  ...
  const targetTime = Math.min(
    Number(freemintStart),
    Number(whitelistStart),
    Number(publicStart)
  );
